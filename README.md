...
**DODANE — tryb „Wybór miast"**: trzeci `Tryb` obok „Cały Region”/„Pojedyncze miasto” — dowolna kombinacja miast z listy (`gr.Dropdown(multiselect=True)`), niezależna od sztywnych grup w `REGIONS_MAP` (np. Kraków + Gdańsk + Warszawa naraz, mimo że są w trzech różnych regionach). Pusty wybór (użytkownik odznaczył wszystko) daje fallback na `Krakow_Centrum` z ostrzeżeniem w Dzienniku, zamiast zwracać pustą tabelę bez wyjaśnienia. `run_simulation()` dostał nowy parametr `selected_cities: list[str] | None`.

**NAPRAWIONE — chipy „Miasta (wybór)” łamały się w połowie słowa**: w wąskiej (190px) lewej kolumnie nazwy miast w chipach multiselect wyglądały jak „Krakow_Cen” / „trum” na dwóch liniach. Przeniesione do szerokiej prawej kolumny (nad tabelą wyników) — tam jest miejsce na pełne nazwy. Dodatkowo `#cities_multi` w CSS wymusza brak łamania w połowie słowa jako druga warstwa zabezpieczenia.

**ZMIENIONE — przycisk „Uruchom prognozę” pod sekcją wyboru trybu**: wcześniej nad `Tryb`, teraz pod `Tryb`/`Region`/`Miasto` — bo wybór trybu (zwłaszcza „Wybór miast”) dzieje się przed uruchomieniem, więc przycisk ma być bezpośrednio pod tym, co się właśnie ustawiło.

**DODANE — przycisk „🔄 Wyczyść cache (EV)”**: obok „Uruchom prognozę”. Czyści `_LAST_PULL` (pamięć poprzednich pulli używana przez `⚡EV`) i usuwa `_last_pull_cache.json` z dysku — przydatne gdy plik zebrał stare wpisy z wcześniejszych stacji/trybów/testów. Nie dotyka `krakow_forecast_snapshots.csv` (to osobna, celowo trwała historia dla `bias_correction`). Po wyczyszczeniu `⚡EV` nie zapali się przy najbliższym pullu (brak z czym porównać) — zacznie znów działać od pulla PO NASTĘPNYM. Funkcja `clear_last_pull_cache()` w `gui_app.py`.

**ZMIENIONE — domyślny tryb startowy**: był `Cały Region`/`poland_south` (7 stacji), teraz `Pojedyncze miasto`/`Krakow_Centrum`. Powód: przy 7 stacjach × do 14 dni (do 98 wierszy) tabela w praktyce nie mieści się w `max_height=700px` — widać głównie pierwszą stację plus kilka wierszy kolejnej, dopóki nie przewinie się w dół, co wygląda jak "region zwraca tylko jedno miasto". To NIE jest błąd zwracania danych — zweryfikowane bezpośrednim wywołaniem `run_simulation("Cały Region", "poland_south", ...)`: backend poprawnie zwraca wszystkie 7 stacji (98 wierszy). Start od pojedynczego miasta daje od razu kompletny widok bez scrolla; tryb „Cały Region” nadal działa poprawnie, tylko trzeba przewinąć tabelę, żeby zobaczyć wszystkie stacje.

**NAPRAWIONE — przycisk „Uruchom prognozę” na dole panelu**: był ostatnim elementem lewej kolumny, pod wszystkimi suwakami i blokami ℹ️ — trzeba było przewinąć, żeby go znaleźć. Przeniesiony na samą górę panelu (nad `Tryb`/`Region`/`Miasto`).
...
**NAPRAWIONE — sygnały TIMDR (⚡ skręt/anomalia/rezonans/defekt) nigdy się nie liczyły**: `run_simulation()` wywoływał `TIMDRAnalyzer(station=node).analyze(df_hist)` bezpośrednio na `df_hist` z `_fetch_historical()` — a to DataFrame z **indeksem czasowym** (`time`) i kolumnami `temp, precip, pressure, wind, humidity` (bez `wind_dir`), podczas gdy `TIMDRAnalyzer`/`WindAnalyzer` są napisane pod schemat z `data/fetcher.py` (kolumna **`datetime`**, kolumna **`wind_speed`**, kolumna `wind_dir`). Efekt: `WindAnalyzer.sudden_direction_change()` wywalał `KeyError: 'wind_dir'` przy pierwszym wywołaniu — ale całość była owinięta gołym `except Exception: pass`, więc błąd znikał bez śladu i `analyze()` po cichu zwracał `{}` (wyglądało jak "przeanalizowano, nic nie znaleziono", a analiza nigdy się nie wykonała). Prawdopodobnie sygnały ⚡ nie zadziałały ani razu od czasu przepisania na GUI. Naprawione: (1) `_OPEN_METEO_HOURLY` dociąga teraz też `winddirection_10m`, `_fetch_historical()` zwraca kolumnę `wind_dir`; (2) nowy adapter `_adapt_for_timdr(df_hist)` w `gui_app.py` zamienia indeks czasowy na kolumnę `datetime` i `wind`→`wind_speed`; (3) `except Exception: pass` zamienione na `except Exception as e: logs.append(f"⚠️ ...: {e}")`, żeby przyszłe regresje tej klasy były widoczne w Dzienniku, a nie znikały. Zweryfikowane bezpośrednim testem (wstrzyknięty skok opadu 80mm wykryty jako `defekt`/`anomalia`).

**NAPRAWIONE — `'precip'` pominięty w analizowanych parametrach TIMDR**: `TIMDRAnalyzer.analyze()` sprawdzał `anomalia`/`defekt`/`skręt`/`rezonans` dla `temp`/`pressure`/`humidity`/`wind_speed`, ale nigdy dla opadów — mimo że to one najbardziej korzystają z wykrywania progowego/skokowego (w odróżnieniu od ekstrapolacji trendu, patrz `_blend_weight()` wyżej, która celowo omija opady). Dodane do listy `params` w `analyzer/timdr_analyzer.py`.

**NAPRAWIONE — progi `AdaptiveThresholds` bezwarunkowo "anomalne" przy pustym `weather_cache.db`**: odkryte przy weryfikacji powyższej naprawy — po podłączeniu prawdziwych danych `analyze()` zwracał absurdalne liczby sygnałów (setki na 168 punktach godzinowych). Przyczyna: `weather_cache.db` (tabele `hourly` i `climatology`) jest zawsze puste w kontekście `gui_app.py` — GUI świadomie pomija cache i bierze dane zawsze świeże z API (patrz docstring modułu) — więc `get_thresholds()` zawsze trafiał w sztywny fallback `{mean:0, std:1, low:-2, high:2}`. Realne wartości ciśnienia (~1013), wilgotności (~0–100) czy temperatury (~10–35) nigdy nie mieszczą się w `[-2, 2]`, więc `is_anomaly()` zwracał `True` bezwarunkowo dla praktycznie każdego wiersza (zweryfikowane: `is_anomaly(1013, ..., 'pressure') == True` zawsze). Naprawa: `AdaptiveThresholds` dostał `fallback_df` — `TIMDRAnalyzer.analyze(df)` ustawia `self.thresholds.fallback_df = df` (te same dane, które i tak są analizowane) na starcie; `get_thresholds()` liczy z niego `mean`/`std`/`p10`/`p90` na żywo, zanim sięgnie po sztywną stałą. To słabsza kalibracja niż prawdziwa klimatologia (bo "normalne" jest zdefiniowane przez samo analizowane okno — nie złapie anomalii obecnej przez całe okno), ale nieporównywalnie lepsza niż stała `[-2, 2]` dopasowana do zupełnie innej skali. Zweryfikowane: liczba `anomalia` spadła z setek do pojedynczych procent próbek, `is_anomaly(1013/60/20, ...)` już nie jest bezwarunkowo `True`, wstrzyknięty skok opadu nadal wykrywany.
